...
| `scan-incomplete` | `ScanResult.truncatedByDeadline` is true and the walk named no folder it could not read: a helper killed inside an ordinary directory or an added root | "BranchBar stopped looking for repos before it finished. What you see may be incomplete. Rescan to try again." | "Rescan" (`rescan`) | — |
| `push-history-unreadable` | The reflog file held a nonempty line the reader could not parse or validate, so the walk stopped at it rather than reporting the push above it | "Push history unreadable" / tooltip "BranchBar found something it could not read in this repo's record of pushes, so it stopped there rather than report a date it cannot stand behind." | "Open in Cursor" | "Copy path" |
| `remote-branches-unread` | `git for-each-ref -- refs/remotes/` failed, so nothing is known about this branch's remote side: neither "No tracked remote branch" nor "In sync" may be claimed | "Push history not checked" / tooltip "BranchBar could not read this repo's branches on origin this time, so it is not saying anything about where this branch stands against them." / "Could not read this repo's last-known origin. Refresh to try again." | "Refresh" (`retryRefresh`) | — |
| `push-not-recorded` | `PushInfo.source == .checkedNoObservation`: `<remote>/<name>`'s record of pushes was read and held no usable line — an untracked branch behind a same-named branch on origin, or a branch whose upstream is gone | "No push from this Mac recorded for origin/feature · newest commit on origin/feature dated 2 days ago" / tooltip "BranchBar read this repo's record of pushes for this branch and found none it can date. It cannot see pushes made from your other computers." | "Open in Cursor" | "Copy path" |
| `push-history-unavailable` | `PushInfo.source == .unavailable`: the repo's own files were not read — a network volume F15 refuses to open descriptors against, or a read that failed. `FetchHeadState.unavailable` rides the same row | "Push history not checked" / tooltip "BranchBar did not read this repo's record of pushes this time, so it is not saying when this branch last went out." / ahead tooltip "Fetch time not checked." | "Open in Cursor" | "Copy path" |
...
| What the scan could not read, and what it skipped on purpose | `scanIncomplete` | `scan-incomplete` — the walk was cut off before it drained its queue, so the repo list is not everything there is. Shown whenever `ScanResult.truncatedByDeadline` is true, including when the walk named no folder it could not read: a helper killed inside an ordinary directory or an added root leaves a truncated result with an empty unreadable list, and suppressing the warning on that emptiness was a silently short list (codex round 3, MAJOR 2). | `BranchBar stopped looking for repos before it finished. What you see may be incomplete. Rescan to try again.` |
...
| Push observation | `pushHistoryUnreadable` | `push-history-unreadable` — the file this clone writes about pushes held a line BranchBar could not vouch for, so it stopped there (codex round 3, MAJOR 7). It is not the `Last push unknown` fallback: that one states a commit date, and stating any date here would be stating it over the corruption that stopped the walk. | `Push history unreadable` |
| Push observation | `pushHistoryUnreadableTooltip` | `push-history-unreadable` — tooltip saying what BranchBar met and what it did. | `BranchBar found something it could not read in this repo's record of pushes, so it stopped there rather than report a date it cannot stand behind.` |
| Push observation | `noPushRecorded` | `push-not-recorded` — BranchBar read this branch's record of pushes for `<remote>/<name>` and found no usable line (codex round 5, MAJOR 3). It replaced `noTrackedRemoteBranch` on every row where the read really happened: a branch with no tracking configuration behind a same-named branch on origin, and a branch whose upstream is missing. Both used to read "push history not checked" under a tooltip saying BranchBar only reads push history for tracking branches — about a branch whose file it had just read. | `No push from this Mac recorded for origin/feature` |
| Push observation | `noPushRecorded` | `push-not-recorded` — the same sentence with the remote tip's commit date appended as what it is: a date about a commit, labelled, never a push. The remote-tip date is the only date this row has, and the fallback names the branch it belongs to so it cannot be read as the local tip's. | `No push from this Mac recorded for origin/feature · newest commit on origin/feature dated 2 days ago` |
| Push observation | `noPushRecordedTooltip` | `push-not-recorded` — tooltip stating what was read and what it holds, without claiming that nothing left this Mac: the record only covers what this clone did. | `BranchBar read this repo's record of pushes for this branch and found none it can date. It cannot see pushes made from your other computers.` |
| Push observation | `pushHistoryUnavailable` | `push-history-unavailable` — the read was skipped or refused, so nothing about this branch's pushes was established (codex round 5, MAJOR 3). The label is the same three words as `pushHistoryNotChecked` because that is exactly what happened; the tooltip differs, because the reason does. | `Push history not checked` |
| Push observation | `pushHistoryUnavailableTooltip` | `push-history-unavailable` — tooltip naming what BranchBar declined to do. A repo on a network volume is read through git alone, because a direct read of a file on a mount whose server has gone away cannot be cancelled (codex round 4, BLOCKER 3). | `BranchBar did not read this repo's record of pushes this time, so it is not saying when this branch last went out.` |
| Push observation | `pushHistoryNotChecked` | `remote-branches-unread` — reading this repo's branches on origin failed, so BranchBar knows nothing about the remote side of this row (codex round 3, MAJOR 6). It may not say there is no matching branch and it may not say the branch is in sync: a failed read proves neither. | `Push history not checked` |
| Push observation | `pushHistoryNotCheckedTooltip` | `remote-branches-unread` — tooltip naming the read that failed, in workshop words. | `BranchBar could not read this repo's branches on origin this time, so it is not saying anything about where this branch stands against them.` |
| Push observation | `aheadTooltip` | `ahead-of-last-known-origin` — tooltip carrying when this clone last heard from origin. `remoteObservedAt` is the `FETCH_HEAD` modification date, which is a local observation; it used to be the remote tip's committer date, so fetching a two-year-old commit today read as "last seen 2 years ago" (codex MAJOR 7). The anchor is a tri-state since codex round 5 MAJOR 4: a nil date used to be read as proof that the repo had never fetched, and F15 refuses the `FETCH_HEAD` read outright on a network volume, so an ahead branch on a file share announced that a repo which fetches daily had never fetched. Only `notFetchedYet` says that now, and it takes an `ENOENT`. | `Counted against last-known origin. This repo's last fetch changed FETCH_HEAD 3 hours ago. BranchBar never fetches, so origin may have moved.` |
| Push observation | `fetchTimeNotChecked` | `fetch-time-not-checked` — the anchor clause when BranchBar did not establish when this repo last fetched: the read was skipped on a network volume, refused, or failed (codex round 5, MAJOR 4). It is not "has not fetched yet", which is a claim about the repo. | `Fetch time not checked.` |
