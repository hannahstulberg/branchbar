...
| `origin-not-fetched` | `Upstream.ahead > 0` and this clone has no `FETCH_HEAD` | tooltip "Counted against last-known origin, origin not fetched by this clone yet. BranchBar never fetches, so origin may have moved." | "Open in Cursor" | "Copy path" |
...
| Push observation | `aheadTooltip` | `ahead-of-last-known-origin` — tooltip carrying when this clone last heard from origin. `remoteObservedAt` is the `FETCH_HEAD` modification date, which is a local observation; it used to be the remote tip's committer date, so fetching a two-year-old commit today read as "last seen 2 years ago" (codex MAJOR 7). | `Counted against last-known origin, last seen 3 hours ago. BranchBar never fetches, so origin may have moved.` |
| Push observation | `originNotFetchedYet` | `origin-not-fetched` — the anchor clause when there is no `FETCH_HEAD` to date. A clone that has only ever been pushed from has never fetched, and saying so is honest where a silent omission read as "we know when, we just did not say". | `origin not fetched by this clone yet` |
