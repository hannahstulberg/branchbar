# synthetic: a repo with six linked worktrees plus a bare mirror, covering every attribute `git worktree list --porcelain` can print
...
Extends the recorded single-worktree output, which this machine's two repos are limited to. Bare repos are out of scope for v1 (PLAN.md §3) but the parser still has to survive the record.
